...
add_action( 'after_setup_theme', 'fire_engine', 16 );

function fire_engine() {
    ...
    // add_filter( 'get_search_form', 'bones_wpsearch' );                               // adding the bones search form (created in functions.php)
    ...
    // Remove Wordpress auto-linebreak-to-p,
    // We'll be doing adding this again later, but after we make sure that
    // shortcodes aren't enclosed.
...
function bones_head_cleanup() {
    ...
    // Create an RSS-feed *without* the 'comments'-rss.
    add_action( 'wp_head', 'eo_create_rss_feed', 1 );
...
function bones_scripts_and_styles() {
  ...
  if (!is_admin()) {
    ...
    */
    ...
    wp_register_script( 'picturefill-picturefill', get_stylesheet_directory_uri() . '/library/js/libs/picturefill/picturefill.js', '', '', true );
    wp_enqueue_script( 'picturefill-picturefill' );  
    wp_register_script( 'picturefill-matchmedia', get_stylesheet_directory_uri() . '/library/js/libs/picturefill/matchmedia.js', '', '', true );
    wp_enqueue_script( 'picturefill-matchmedia' );  
